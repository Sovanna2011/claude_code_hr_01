...
```
┌──────────────────────────┐     HTTPS/JSON      ┌──────────────────────────┐    SQL/PL-SQL  ┌─────────────────┐
│      SAPUI5 (Fiori)       │  ───────────────▶   │   ASP.NET Core Web API    │  ──────────▶   │  Oracle Database │
...
| Application  | C# / ASP.NET Core 8, EF Core 8 (Oracle provider) | REST API, infotype read on key date, SAP "delimit + new time slice" logic, org path evaluation |
| Persistence  | Oracle Database (19c/21c/23ai) | SAP-faithful schema (PA infotypes, HRP1000/1001, T-tables), PL/SQL procedures, reporting views |
...
## Oracle porting notes

This variant runs on **Oracle Database** (the sibling `claude_code_hr_01`
targets SQL Server). The application logic is unchanged — only the persistence
layer and a few provider-sensitive details differ:

- **Provider** — `Oracle.EntityFrameworkCore` with `opt.UseOracle(...)`; the
  connection string is Oracle EZConnect (`User Id=HR;…;Data Source=host:1521/SVC`).
- **Schema = user** — Oracle has no separate "database + schema"; the `HR`
  schema is an Oracle user created by `01_create_schema.sql`. Entities keep
  `ToTable("…","HR")`.
- **Identifier folding** — Oracle stores unquoted identifiers in UPPERCASE while
  EF Core quotes and case-preserves the SQL it generates. `HRDbContext` upper-cases
  every table/schema/column name so the provider SQL matches the clean, unquoted
  DDL in `database/*.sql`.
- **Empty string is NULL** — Oracle treats `''` as `NULL`, so the SAP-initial
  value of the mandatory key fields `SUBTY`/`OBJPS`/`SPRPS` is a single space
  (`InfotypeBase` and the DDL `DEFAULT ' '`), keeping the composite PK non-null.
- **Type mapping** — `INT→NUMBER(10)`, `BIGINT→NUMBER(19)`,
  `VARCHAR/NVARCHAR→VARCHAR2/NVARCHAR2`, `DECIMAL→NUMBER`, `BIT→NUMBER(1)`,
  `DATETIME2(0)→TIMESTAMP(0)`, `TIME(0)→INTERVAL DAY TO SECOND` (maps to
  `TimeSpan`), `IDENTITY→GENERATED BY DEFAULT ON NULL AS IDENTITY`.
- **SQL dialect** — `MERGE … USING (SELECT … FROM DUAL)` for idempotent seeds,
  PL/SQL `EXECUTE IMMEDIATE` blocks that swallow `ORA-00955` for re-runnable DDL,
  PL/SQL procedures with `OUT`/`SYS_REFCURSOR` parameters, recursive `WITH` for
  the org tree, and an NLS-neutral virtual column for the overtime category.
